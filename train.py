        ...
        if i % opt.valInterval == 0:
            ...
            with open(f'./saved_models/{opt.experiment_name}/log_train.txt', 'a') as log:
                ...
                if current_accuracy > best_accuracy:
                    ...
                    torch.save(model.state_dict(), f'./saved_models/{opt.experiment_name}/best_accuracy_{i:08d}.pth')
                if current_norm_ED < best_norm_ED:
                    best_norm_ED = current_norm_ED
                    torch.save(model.state_dict(), f'./saved_models/{opt.experiment_name}/best_norm_ED_{i:08d}.pth')
...
if __name__ == '__main__':
    ...
    if not opt.experiment_name:
        opt.experiment_name = f'{datetime.now().strftime("%Y_%m_%d_%H_%M")}'
    ...
    opt.character = '#&*;\\^_`\'+=><@[|]%$~!"(),-./0123456789:?ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz{}ÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚÝàáâãèéêìíòóôõùúýĂăĐđĨĩŨũƠơƯưẠạẢảẤấẦầẨẩẪẫẬậẮắẰằẲẳẴẵẶặẸẹẺẻẼẽẾếỀềỂểỄễỆệỈỉỊịỌọỎỏỐốỒồỔổỖỗỘộỚớỜờỞởỠỡỢợỤụỦủỨứỪừỬửỮữỰựỲỳỴỵỶỷỸỹ'
